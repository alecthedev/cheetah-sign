...
| ---------- | ------------------------------------------- | ------------------------------------------------------------------ | --------------- | -------------- |
| 2026-04-15 | Document Upload                             | Accepted upload + conversion behavior for supported types          | None            | Y              |
| 2026-04-15 | Document Signing                            | Accepted signer flow, validation, and multi-document lock behavior | None            | Y              |
| 2026-04-15 | Download Completed Job/Packet + Certificate | Accepted downloaded output format and certificate inclusion        | None            | Y              |
| 2026-04-15 | Document Builder + Packet                   | Accepted packet creation/reorder and field placement workflow      | None            | Y              |
| 2026-04-15 | Client + Job Dashboards                     | Accepted client validation and job filtering/search/audit behavior | None            | Y              |
...
- Risk: Endpoint-level tests using localhost dependencies can be environment-sensitive during demos.
- Mitigation Plan: Use deterministic seeded test data and run unit/integration test suites before client walkthrough.

## Risk2

- Risk: Very large files and unsupported formats can still be attempted by users and create support requests.
- Mitigation Plan: Keep explicit UI messaging for size/type constraints and include demo of rejection behavior.

## Risk3

- Risk: Packet and signer complexity (overrides, ordering, parallel toggle) can cause configuration mistakes by users.
- Mitigation Plan: Include a UAT script that verifies signer assignments and signing order before send.

---

# 5. Spot Check (3 Feature Requirement)

For each selected feature:

| Spot Check Feature                          | Full Flow Documented | Corresponding Test(s)                                                        | Tests Pass During Demo                                         |
| ------------------------------------------- | -------------------- | ---------------------------------------------------------------------------- | -------------------------------------------------------------- |
| Document Upload                             | Yes                  | `UploadDocumentTests.cs`, `DocumentConverterTests.cs`, `upload-form.test.ts` | Yes                                                            |
| Document Signing                            | Yes                  | `document-signing.test.ts`                                                   | Yes                                                            |
| Download Completed Job/Packet + Certificate | Yes                  | `FileDeleterAndPacketDownloadTests.cs`, `job-display.test.ts`                | Partially (web tests pass; API tests blocked by local DB auth) |

## Demonstration Test Commands

- Web signing flow tests: `npm run test -- src/unit-tests/document-signing.test.ts`
- Web upload flow tests: `npm run test -- src/unit-tests/upload-form.test.ts`
- API upload/download flow tests: `dotnet test --filter "FullyQualifiedName~UploadDocumentTests|FullyQualifiedName~DocumentConverterTests|FullyQualifiedName~FileDeleterAndPacketDownloadTests"`

## Latest Execution Notes

- `npm run test -- src/unit-tests/upload-form.test.ts src/unit-tests/document-signing.test.ts` -> Passed (77/77 tests).
- `dotnet test --filter "FullyQualifiedName~UploadDocumentTests|FullyQualifiedName~DocumentConverterTests|FullyQualifiedName~FileDeleterAndPacketDownloadTests"` -> Blocked by local PostgreSQL authentication (`28P01: password authentication failed for user "admin"` in `PgDataFixture` setup).
